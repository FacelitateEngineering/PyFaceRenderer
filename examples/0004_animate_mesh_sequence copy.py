...
folders = list(filter(lambda f: f.is_dir(), animation_file.glob("*")))
for folder in folders:
    ...
    if len(objs) == 0:
        ...
        exit(1) 
    output_filename = (Path('tmp') / folder.stem).with_suffix('.pickle')
    log.info(f'Rendering animation: {len(objs)} frames from {folder.stem} -> {output_filename}')
